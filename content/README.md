...
Photography / Architecture manifest 还负责归档交互策略：`initialVisibleCount` 控制首屏数量（Architecture 可写 `all`），`loadMoreBatchSize` 控制滚动到归档底部时自动加载的批次大小，`eagerImageCount` 控制首屏优先加载的图片数量。若浏览器不支持 `IntersectionObserver` 或关闭 JavaScript，页面会一次性显示全部记录，避免隐藏内容没有可操作入口。
...
正文远程图片的实际宽高不允许猜测：已确认值写入 `content/site/blog-image-dimensions.json`，可用 `npm run blog:images:dimensions` 查看待确认清单；网络和来源许可都满足时再运行 `node scripts/prepare-blog-image-dimensions.mjs --write`。
