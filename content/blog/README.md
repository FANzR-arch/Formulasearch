...
正文图片 alt 的维护：旧正文可先运行 `npm run blog:images:prepare`，它只会把空 alt 或“图像”替换成最近章节标题 + “配图”的中文初稿，不会覆盖已有具体描述。提交前运行 `npm run blog:images:report`，查看哪些图片仍是这种上下文初稿；它们需要结合实际画面逐张改成具体描述，再运行 `npm run blog:images:check`。报告只输出到终端，不会生成需要长期维护的快照文件。
