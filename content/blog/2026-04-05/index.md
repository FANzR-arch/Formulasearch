...
翻译成人话：**一台笔记本，本地1 Petaflop。** 不需要工作站，不需要机房，不需要把设计图纸上传到任何人的服务器。

更值得关注的是这套Agent的技术栈：

- **AI框架**：Hermes harness，开放式沙盒架构。Agent在沙盒里被赋予工具调用权限，能自主操作Rhino、Blender、Flux 2
- **本地模型**：Nemotron 3 Ultra，NVIDIA最新的开源大模型，负责执行层面的任务调度
- **云端调用**：Claude Sonnet，遇到复杂推理时混合调用
- **涉及软件**：Rhino（建模）+ Blender（渲染）+ Flux 2（图像生成）
...
我在AI行业这一年多，看到的最大趋势是什么？不是模型变强了，是**工具在被Agent编排**。
...
这些东西AI学不会。不是因为它不够聪明。是因为这些东西**没有标准答案**。

**对场地的理解，对材料的感知，对人在空间里会怎么走的直觉，对一栋建筑应该长什么样的判断。**
