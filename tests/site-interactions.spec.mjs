...
test('partners page exposes its mutual site as a safe external link', async ({ page }) => {
  ...
  await expect(partner.nth(0).locator('strong')).toContainText('水土营造')
  await expect(partner.nth(1)).toHaveAttribute('href', 'https://www.syntaxstudio.work/')
...
test('photo selection accordion supports pointer and keyboard selection', async ({ page }) => {
  ...
  await expect(page.locator('.photo-accordion__label')).toHaveCount(0)
