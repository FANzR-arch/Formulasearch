...
> 更新：2026-08-11
> 状态：Phase 0、Phase 1 与详情页骨架已完成；16 篇正文已迁移，另有 10 篇保留为外链索引；封面尺寸清单与响应式 WebP 已接入构建门禁，关联网络与更深层图片管线待后续推进。
...
3. 在现有 Astro 内容集合、SEO 与 sitemap 基础上继续完善文章关联网络（详情页自动相关内容已完成，后续可补 series/tags 元数据提高精度）。
...
2. 内容集合稳定后：评估把封面迁到可处理资源目录，使用 Astro 图片组件生成多尺寸 WebP/AVIF；当前已用独立 Sharp 清单完成 WebP 过渡，不复用旧硬编码任务表。
