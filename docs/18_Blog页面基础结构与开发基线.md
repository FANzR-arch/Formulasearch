...
> 更新：2026-08-11
> 状态：Phase 0、Phase 1 与详情页骨架已完成；16 篇正文已迁移，另有 10 篇保留为外链索引；封面尺寸清单与构建门禁已完成，真正的响应式图片优化与关联网络待后续推进。
...
3. 在现有 Astro 内容集合、SEO 与 sitemap 基础上继续完善文章关联网络。
...
本阶段新增 `scripts/prepare-blog-media.mjs`：它从实际封面读取宽高和文件大小，生成 `content/site/blog-media.json`；`blog:media:check` 已接入构建，文章封面、首页精选舞台和列表封面均输出 `width`、`height` 与 `sizes`。
