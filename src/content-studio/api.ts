...
import { deletePhoto, importPhotoFiles, importPhotoFolder, readPhotoArchive, updatePhotoItems, PhotoConflictError } from './photo-manager.mjs'
...
const startTask = createTaskRunner({ projectRoot, stateRoot })
  ...
  try {
    if (params.action === 'photos') {
      ...
      if (contentType.includes('multipart/form-data')) {
        ...
        const uploads = path.join(stateRoot, 'photo-upload')
        await fsp.mkdir(uploads, { recursive: true })
        const uploadRoot = await fsp.mkdtemp(path.join(uploads, 'upload-'))
        const paths: string[] = []
        try {
          ...
          return json(await importPhotoFiles(paths))
        } finally { await fsp.rm(uploadRoot, { recursive: true, force: true }) }
      }
      const payload = await request.json()
      if (payload.action === 'import-folder') return json(await importPhotoFolder(payload.path))
      if (payload.action === 'update') return json(await updatePhotoItems(payload.items, payload.revision))
      if (payload.action === 'delete') return json(await deletePhoto(payload.assetHash))
      throw new Error('未知照片操作。')
    }
    if (params.action === 'prepare') return json(await startTask('prepare'))
    if (params.action === 'publish') {
      ...
      return json(await startTask('publish', {
    ...
    return json({ error: error instanceof Error ? error.message : String(error) }, error instanceof PhotoConflictError ? 409 : 400)
  }
}
